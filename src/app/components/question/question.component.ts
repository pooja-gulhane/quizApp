...
export class QuestionComponent implements OnInit {
  ...
  seconds: number = 0;
  interval: any;
  localStorageKey: string = 'quizTimer_' + this.quizId; // Key for storing time in localStorage
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.quizService.getQuizDetails(this.quizId).subscribe(
        (quiz) => {
          this.totalTime = quiz.quizTimeAllocated * 60; // Assuming totalMarks is a field in your response
  ...
  ngOnDestroy() {
    // Save the remaining time to localStorage before the component is destroyed
    this.saveTimer();
  ...
  onSubmit(questionIndex: any): void {
    ...
    clearInterval(this.interval); // Stop the timer once the quiz is submitted
